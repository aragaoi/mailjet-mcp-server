...
export const API_KEY = process.env.MAILJET_API_KEY;
// Alternate non-US region of the API server
export const API_REGION = process.env.MAILJET_API_REGION?.toLowerCase();
// API server hostname based on region
export const API_HOSTNAME = `api.${API_REGION ? `${API_REGION}.` : ''}mailjet.com`;
 ...
 */
export function extractEndpoints(openApiSpec) {
  try {
...
export function resolveReference(ref, openApiSpec) {
  ...
  return refPath.reduce((obj, path) => obj[path], /** @type {any} */ (openApiSpec));
}
 ...
 * @param {NonNullable<ReturnType<typeof getOperationDetails>>['operation']} operation - OpenAPI operation object
 */
export function registerTool(server, toolId, toolDescription, paramsSchema, method, path, operation) {
